...
export default function startCronJobs() {
  if (process.env.NODE_ENV === "production") {
    ...
    const updateGames = cron.schedule(
      ...
      //"*/5 * * * *",
      async () => {
        console.log("Creating new daily games....");
        ...
        console.log("Finished making new games");
      },
      {
        timezone: "America/New_York",
      }
    ...
    const calculateStats = cron.schedule(
      `0 5 * * *`,
      async () => {
        ...
        try {
          ...
          for (let i = 2; i <= 7; i++) {
            try {
              ...
              console.log("1. Somewent went wrong calculating daily stats");
            }
          }
          await game_stats.updateOne(
            { name: "averages" },
          ...
          console.log("Finished creating stats");
        } catch {
          console.log("2. Something went wrong calculating daily stats");
        }
      },
      {
        timezone: "America/New_York",
